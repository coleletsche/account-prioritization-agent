...
export const BriefingRequestSchema = z.object({
  ...
  accounts: z.array(z.object({
    ...
    factors: z.object({ intent: z.number().min(0).max(100), value: z.number().min(0).max(100).nullable(), timing: z.number().min(0).max(100).nullable() }).strict(),
  ...
  return BriefingRequestSchema.parse({
    ...
    accounts: accounts.slice(0, 40).map((account) => {
      ...
      return {
        ...
        score: account.score,
        factors: { intent: account.factors.intent, value: account.factors.value ?? null, timing: account.factors.timing ?? null },

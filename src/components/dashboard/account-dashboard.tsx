...
import { AlertTriangle, CalendarDays, Database, FileWarning, Info, Search, Signal, SlidersHorizontal, Users } from "lucide-react";
...
function metric(value: string, label: string, detail: string, icon: React.ReactNode, tone = "default") {
  return { value, label, detail, icon, tone };
}
...
export function AccountDashboard() {
  ...
  const signalCoverage = Math.round((data.organizations.filter((organization) => organization.engagements.length > 0).length / data.organizations.length) * 100);
  const metrics = [
    metric(String(ranked.length), "eligible accounts", `${data.statistics.excludedOrganizations} held for review`, <Users size={19} />),
    metric(data.latestEngagementDate ?? "—", "latest engagement", stale ? `${freshnessDays} days before this week` : "Source data is current", <CalendarDays size={19} />, stale ? "warning" : "default"),
    metric(`${signalCoverage}%`, "signal coverage", `${data.statistics.matchedSignals} matched engagement rows`, <Signal size={19} />),
    metric(String(data.reviewQueue.length), "quality flags", "Every issue remains inspectable", <FileWarning size={19} />),
  ];

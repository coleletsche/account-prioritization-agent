...
export function IntakeWorkspace({ phase, stage, onAnalyze, onValidatingChange }: { phase: Exclude<WorkspacePhase, "dashboard">; stage: AnalysisStage; onAnalyze: (data: EntityResolutionResult, label: string) => Promise<void>; onValidatingChange: (validating: boolean) => void }) {
        ...
        {!analyzing ? (
          <div className="intake-workspace-grid">
            <section className="intake-primary-card">
              ...
              <p className="mt-4 max-w-2xl text-base leading-7 text-muted">Add the two CRM exports. Velora will validate identity, calculate fixed priority scores, and prepare policy-checked next actions.</p>
          ...
          <section className="analysis-card" aria-live="polite" aria-labelledby="analysis-title">
            ...
            <ol className="analysis-steps mt-8">
              {ANALYSIS_STEPS.map((label, index) => {
                ...
                return <li key={label} className={active ? "analysis-step-active" : complete ? "analysis-step-complete" : ""}><span>{complete ? <Check size={15} /> : active ? <LoaderCircle className="animate-spin" size={16} /> : <Circle size={13} />}</span><strong>{label}</strong><small>{complete ? "Complete" : active ? "In progress" : "Waiting"}</small></li>;
              })}
            </ol>
